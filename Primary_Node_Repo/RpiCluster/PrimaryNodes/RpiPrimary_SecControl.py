...
class RpiPrimary:
    ...
    def __init__(self, socket_bind_ip, socket_port, agentConfigFile): #, influxdb_host, influxdb_port, influxdb_database_prefix
        ...
        self.number_of_secondary_nodes = 15
    ...
    def send_Nbr_info_to_secondary(self):
        ...
        for nodeID in self.connected_nodeIDs:
            ...
            # logger.info("NbrIDs =" +str(myNbrslist))
    ...
    def max_cons_init(self, Max_init_vec):
        """Allows retrieving the solution of every secondary connected to the primary"""
        Nodes_Max_Init_vec = np.zeros(self.number_of_secondary_nodes)  
        
        Max_init_vec = np.array(Max_init_vec)        
        ...
        for nodeID in self.connected_nodeIDs:
            ...
            Nodes_Max_Init_vec_dummy = np.zeros(len(myNbrslist))
            
            for nbrNode in range(len(myNbrslist)):
                Nodes_Max_Init_vec_dummy[nbrNode] = np.abs( Max_init_vec[nodeID] - Max_init_vec[myNbrslist[nbrNode]] ) 

            Nodes_Max_Init_vec[nodeID] = Nodes_Max_Init_vec_dummy.max()

        return Nodes_Max_Init_vec
    ...
    def send_max_cons_val_to_agents(self, max_consensus_value):  
        for nodeID in self.connected_nodeIDs:
            myconnection = self.connected_client_sockets[nodeID]
            myconnection_handler = ConnectionHandler(myconnection)
            myconnection_handler.send_message(max_consensus_value, "max_consensus_value")            
        # logger.info("new max consensus convgence value = " +str(max_consensus_value))
    ...
    def send_max_stop_to_agents(self):
        ...
        # logger.info("sent stop communication for max cons to all the agents" +str(mystopflag))
    ...
    def send_max_init_stop_to_agents(self):
        ...
        # logger.info("sent stop communication for max cons initialization to all the agents" +str(mystopflag))
